...
#INPUT file_PTMs.txt(PTMs conocidas) file_TFBSs.gff(coordenadas) file_exp.bed(coordenadas experimento a analizar)
#OUTPUT output.txt 
...
open FILE,">out/output_filter_PTMs.txt";
...
for (my $q = 3; $q < scalar @ARGV; $q++){
	...
	if(exists $hash_PTMs{$ARGV[$q]}){
		...
		print "$ARGV[$q] not found the configuration file\n";
